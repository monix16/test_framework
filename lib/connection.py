...
log = logging.getLogger("plivo_connection")


class Connection:

    def __init__(self, auth_id, auth_token, rest_url, skip_ssl_cert_check):
    ...
    def _api_call_raw(self, req_type, path, data=None, params=None):
        ...
        kwargs = {'headers': self._headers, 'auth': HTTPBasicAuth(self.auth_id, self.auth_token), 'verify': not self.skip_ssl_cert_check}
